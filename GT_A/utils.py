...
- Classes to run the GT algorithm on both quadratic and logistic functions
"""
...
class Dataset:
    ...
    def run(self):
        """ Runs the GT algorithm """
        if not hasattr(self, 'history'):
            Theta = np.random.randn(self.m, self.d)
        else:
            Theta = self.history[0, 0]

        B = np.zeros((self.m, self.d))
        
        history = np.zeros((self.n_iter + 1, 2, self.m, self.d))
        history[0][0] = Theta
        history[0][1] = B
        ...
        for i in range(self.n_iter):
            ...
            Theta = self.W @ (Theta - self.gamma * grad - self.gamma * B)
            B = self.W @ B - (np.eye(self.m) - self.W) @ grad
    ...
    def plot_distance(self, idx_end = -1, legend = True, save = None):
        ...
        history_dist_star = ((self.history[:, 0] - self.theta_star[np.newaxis, :])**2).sum((1, 2))

        plt.figure(figsize = (8, 5))
        idx_start, idx_end = 10, idx_end
        plt.plot(history_dist_star[idx_start:idx_end])
        plt.title("Distance to the optimum")
        plt.xlabel("Iteration t")
        plt.ylabel(r"$\|\Theta_t - \Theta^*\|^2$")
        plt.grid()
        if legend:
          plt.legend()
        if save is not None:
    ...
    def plot_2D(self, ax = None, title = None, interval = 1, legend = True, save = None, delta = .5, end = -1):
      ...
      for i in range(self.m):
          ax.scatter(self.history[0, 0, i, 0], self.history[0, 0, i, 1], color=self.colors[i], s=50, linewidth=1,
                    label='Agent start' if i == 0 else "", alpha=0.4, marker='x')
          ax.plot(self.history[:end][::interval, 0, i, 0], self.history[:end][::interval, 0, i, 1], color=self.colors[i], marker='o', markersize=1,
                  linestyle='-', alpha=0.4)
      for i in range(self.m):
      ...
      ax.scatter(self.theta_star[0], self.theta_star[1], color='blue', label=r'Global optimum $\theta^{*}$', zorder=6, s=200,
                edgecolors="black", linewidth=1, marker='^', alpha=1.0)
      ...
      x_min, x_max = self.theta_star[0] - delta, self.theta_star[0] + delta
      y_min, y_max = self.theta_star[1] - delta, self.theta_star[1] + delta
...
class LogisticDataset(Dataset):
    ...
    def get_theta_star(self, n_iter = 1000, tol = 1e-8):
